...
var
  card: tuple[order: string, suit: string]   # Tuples have *both* field names
  quote: tuple[sentence: string, author: string]
...
drinks = @["Caocao", "Liubei", "Sunquan"] # @[V1,..,Vn] is the sequence literal

drinks.add("Simayi")

if "Simayi" in drinks:
  echo "simayi ended", drinks.len - 1, " kingdoms"
...
func iseven(i: int):bool = return (i mod 2 == 0)

for i, c in myString:       # Index and letter. Or `for j in` for just letter
  if iseven(i): continue # Compact `if` form
  elif c == 'X': break
  else: echo(c)
